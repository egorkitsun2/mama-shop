...
function handleImgError(img) {
  ...
  placeholder.className =
    "w-full h-28 bg-gray-50 dark:bg-gray-900 rounded-lg mb-2 flex items-center justify-center text-gray-300 dark:text-gray-600 text-xs border border-dashed border-gray-200 dark:border-gray-700";
  placeholder.innerText = "Нет фото";
...
function getEmptyStateHTML(
  ...
  return `
    <div class="col-span-full my-4 bg-gray-50 dark:bg-gray-800/60 border border-gray-200/80 dark:border-gray-700 rounded-2xl p-6 text-center relative overflow-hidden shadow-sm">
      ...
      <h3 class="text-sm font-bold text-gray-800 dark:text-gray-200 mb-1">${escapeHtml(title)}</h3>
      <p class="text-xs text-gray-500 dark:text-gray-400 max-w-xs mx-auto leading-relaxed">${escapeHtml(desc)}</p>
...
async function loadProducts() {
  ...
  try {
    ...
    document.getElementById("catalog-container").innerHTML =
      `<p class="text-center text-red-500 py-8 text-sm">Ошибка загрузки: ${escapeHtml(error.message)}</p>`;
  }
}
...
function renderCategories() {
  ...
  categoriesNav.innerHTML = availableCategories
    .map(
      (cat) => `
      <button onclick="setCategory('${escapeHtml(cat)}')" class="px-3 py-1 text-xs font-semibold rounded-full whitespace-nowrap border transition-all ${
        cat === activeCategory
          ? "bg-green-600 text-white border-green-600 shadow-sm"
          : "bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700"
      }">
        ${cat === "all" ? "Все товары" : escapeHtml(cat)}
      </button>
    `,
    )
    .join("");
...
function renderSubcategories() {
  ...
  subNav.innerHTML = allSubcats
    .map(
      (sub) => `
      <button onclick="setSubcategory('${escapeHtml(sub)}')" class="px-2.5 py-0.5 text-[11px] font-medium rounded-full whitespace-nowrap border transition-all ${
        ...
        ${sub === "all" ? "Все подкатегории" : escapeHtml(sub)}
      </button>
    `,
    )
    .join("");
        ...
        <div onclick="openProductModal('${item.barcode}')" class="snap-center shrink-0 w-72 sm:w-80 bg-gradient-to-r from-red-500 to-orange-500 rounded-2xl p-3.5 text-white shadow-md relative overflow-hidden cursor-pointer active:scale-98 transition-transform">
          ...
          <div class="flex items-center gap-3">
            <div class="w-20 h-20 bg-white dark:bg-gray-800 rounded-xl shrink-0 p-1 flex items-center justify-center">
              ${
                mainImg
                  ? `<img src="static/img/${mainImg}" loading="lazy" onerror="handleImgError(this)" class="w-full h-full object-contain">`
            ...
            <div class="flex-grow min-w-0">
              ...
              <h4 class="font-bold text-xs sm:text-sm leading-tight truncate mt-1">${escapeHtml(item.name)}</h4>
...
function renderCardHTML(item) {
  ...
  const noImgHTML = `<div class="w-full h-28 bg-gray-50 dark:bg-gray-900 rounded-lg mb-2 flex items-center justify-center text-gray-300 dark:text-gray-600 text-xs border border-dashed border-gray-200 dark:border-gray-700">Нет фото</div>`;
    ...
    <article onclick="openProductModal('${item.barcode}')" class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-2.5 flex flex-col justify-between hover:shadow-md transition-all border border-gray-100 dark:border-gray-700/60 h-full cursor-pointer relative group">
      <div>
        <div class="relative">
          ${
            mainImg
              ? `<img src="static/img/${mainImg}" alt="${escapeHtml(item.name)}" loading="lazy" onerror="handleImgError(this)" class="w-full h-28 object-cover rounded-lg mb-2 bg-gray-50 dark:bg-gray-900">`
              : noImgHTML
          }
          ${
            hasDiscount
        ...
        <div class="flex flex-wrap gap-1 mb-1">
          <span class="text-[9px] font-semibold bg-green-50 dark:bg-green-950/60 text-green-700 dark:text-green-300 px-1.5 py-0.5 rounded">${escapeHtml(item.category || "Прочее")}</span>
          ${item.volume_weight ? `<span class="text-[9px] font-semibold bg-blue-50 dark:bg-blue-950/60 text-blue-700 dark:text-blue-300 px-1.5 py-0.5 rounded">${escapeHtml(item.volume_weight)}</span>` : ""}
        </div>
        <h3 class="font-semibold text-xs sm:text-sm text-gray-800 dark:text-gray-100 leading-tight group-hover:text-green-600 dark:group-hover:text-green-400 transition-colors">${escapeHtml(item.name)}</h3>
  ...
  categories.forEach((cat) => {
    ...
    if (itemsInCat.length === 0) {
      html += `
        <section class="mb-6">
          <div class="flex justify-between items-center mb-2">
            <h2 class="text-base font-bold text-gray-800 dark:text-gray-200"><span>${escapeHtml(cat)}</span></h2>
          </div>
    ...
    html += `
      <section class="mb-6">
        <div class="flex justify-between items-center mb-2">
          ...
          <button onclick="setCategory('${escapeHtml(cat)}')" class="text-xs font-bold text-green-600 dark:text-green-400 hover:underline">Все ➔</button>
        </div>
...
function switchModalImage(imgName, el) {
  const mainImg = document.getElementById("modalMainImg");
  if (!mainImg) return;

  mainImg.src = `static/img/${imgName}`;

  document.querySelectorAll(".modal-thumb-btn").forEach((btn) => {
    btn.classList.remove("border-green-500", "ring-2", "ring-green-500/30");
    btn.classList.add("border-gray-200", "dark:border-gray-700");
  });

  if (el) {
    el.classList.remove("border-gray-200", "dark:border-gray-700");
    el.classList.add("border-green-500", "ring-2", "ring-green-500/30");
  }
}
...
function openProductModal(barcode) {
  ...
  if (imagesList.length > 0) {
    ...
    if (imagesList.length > 1) {
      thumbsContainer.innerHTML = imagesList
        .map(
          (img, idx) => `
            <button
              type="button"
              onclick="switchModalImage('${img}', this)"
              class="modal-thumb-btn w-12 h-12 rounded-lg border-2 ${
                idx === 0
                  ? "border-green-500 ring-2 ring-green-500/30"
                  : "border-gray-200 dark:border-gray-700"
              } overflow-hidden shrink-0 bg-white dark:bg-gray-800 transition-all active:scale-95"
            >
              <img src="static/img/${img}" class="w-full h-full object-cover">
            </button>
          `,
        )
        .join("");
    ...
    .map((b) => {
      ...
      const mainImg =
        ...
        (Array.isArray(product.images) && product.images.length > 0
          ? product.images[0]
          : product.image);
      const imgPath = mainImg ? `static/img/${mainImg}` : null;
      ...
      return `
        <div class="flex items-center justify-between bg-gray-50 dark:bg-gray-900/80 p-2.5 rounded-xl border border-gray-200/80 dark:border-gray-700">
          <div class="flex items-center gap-2.5 flex-grow min-w-0 pr-2">
            ...
            <div class="min-w-0">
              <h4 class="font-semibold text-xs text-gray-800 dark:text-gray-200 truncate leading-tight">${escapeHtml(item.name)}</h4>
